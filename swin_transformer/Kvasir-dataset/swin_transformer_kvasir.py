...
class Config:
    ...
    data_root = "/home/agauta01/anmol_work/kvasir-dataset-v2"
    features_root = "/home/agauta01/anmol_work/kvasir-dataset-v2-features"
    output_dir = "/home/agauta01/work/kvasir_swin_results"
    
    # Create split directories (will be created by the script)
    split_data_root = "/home/agauta01/anmol_work/kvasir-dataset-v2-split"
    
    # Model configuration
    model_name = "swin_base_patch4_window12_384"
    pretrained = True
    pretrained_weights_path = "/home/agauta01/work/swin_base_weights/swin_base_patch4_window12_384.pth"
    num_classes = 8  # Updated for Kvasir-v2
    img_size = 384
    
    # Training hyperparameters
    batch_size = 16
    num_epochs = 60  # Max 60 epochs as requested
    learning_rate = 1e-4
    weight_decay = 0.05
    warmup_epochs = 5
...
def create_data_splits():
    """
    Create train/val/test splits from the original Kvasir dataset
    ...
    splits = ['train_set', 'validation_set', 'test_set']
    for split in splits:
        split_dir = Path(Config.split_data_root) / split
    ...
    for class_name in Config.class_names:
        ...
        class_dir = Path(Config.data_root) / class_name
        ...
        for img_file in train_files:
            dest = Path(Config.split_data_root) / 'train_set' / class_name / img_file.name
            shutil.copy2(img_file, dest)
        
        for img_file in val_files:
            dest = Path(Config.split_data_root) / 'validation_set' / class_name / img_file.name
            shutil.copy2(img_file, dest)
        
        for img_file in test_files:
            dest = Path(Config.split_data_root) / 'test_set' / class_name / img_file.name
            shutil.copy2(img_file, dest)
...
class KvasirDataset(Dataset):
    """
    Custom dataset for Kvasir-v2 gastrointestinal images.
    ...
    def __init__(self, root_dir, split='train_set', transform=None):
        ...
        for class_name in self.classes:
            ...
            # Look for .jpg files (Kvasir-v2 format)
            jpg_files = list(class_dir.glob("*.jpg"))
            
            for img_path in jpg_files:
                self.samples.append((str(img_path), self.class_to_idx[class_name]))
            
            print(f"  Found {len(jpg_files)} images in {class_name}")
...
# ============================================================================
def main():
    # Create data splits first (only needs to be done once)
    if not os.path.exists(Config.split_data_root):
        print("Split data not found. Creating train/val/test splits...")
        create_data_splits()
    else:
        print(f"Using existing split data from: {Config.split_data_root}")
    ...
    # Create datasets
    print("\nLoading datasets...")
    train_dataset = KvasirDataset(Config.split_data_root, split='train_set', 
                                  transform=get_transforms('train'))
    val_dataset = KvasirDataset(Config.split_data_root, split='validation_set', 
                                transform=get_transforms('val'))
    test_dataset = KvasirDataset(Config.split_data_root, split='test_set', 
    ...
    plt.title('Confusion Matrix - Kvasir-v2 Dataset')
